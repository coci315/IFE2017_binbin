...
var delBtn = document.querySelector('.delete>button');
...
document.addEventListener('click', function (e) {
  ...
  if (className) {
    if (className == 'parent' || reg.test(className)) {
      selectNode = target;
      traverseNode01(root, function (node) {
        node.style.backgroundColor = '#fff';
      });
      target.style.backgroundColor = '#f8e000';
    }
  }
});
...
addBtn.addEventListener('click', function () {
  if (selectNode && addInput.value.trim() !== '') {
    var div = document.createElement('div');
    div.classList.add('child');
    div.innerText = addInput.value.trim();
